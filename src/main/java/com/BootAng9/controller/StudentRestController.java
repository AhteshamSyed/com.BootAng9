package com.BootAng9.controller;

import com.BootAng9.model.Message;
import com.BootAng9.model.Student;
import com.BootAng9.service.IStudentService;
        ...
        @RequestBody Student student)
{
    ...
    try {
        ...
        resp= new ResponseEntity<Message>
                (new Message("SUCCESS", id + "-saved"), HttpStatus.OK);
    } catch (Exception e) {
        resp=new ResponseEntity<Message>
    ...
    public ResponseEntity<?> getAllStudents(){
        ...
        try {
            ...
            resp=new ResponseEntity<String>
                    ("Unable to fetch Data",HttpStatus.INTERNAL_SERVER_ERROR);
            e.printStackTrace();
        }
        return resp;
    }
...
*/
    ...
    { ResponseEntity<?> resp= null;
        try {
            ...
            if(opt.isPresent()) resp=new ResponseEntity<Student>(opt.get(),HttpStatus.OK);
            else resp=new ResponseEntity<String>("No Data Found",HttpStatus.BAD_REQUEST);
        } catch (Exception e) {
            resp=new ResponseEntity<String>
                    ("Unable to Fetch Data",HttpStatus.INTERNAL_SERVER_ERROR);
            e.printStackTrace();
            ...
            @PathVariable Integer id)
    {
        ...
        try {
            ...
            }else {resp=new ResponseEntity<Message>
                    (new Message("FAIL",id+"-Not Exist"),HttpStatus.BAD_REQUEST);
            }
        } catch (Exception e) {
            resp=new ResponseEntity<Message>
                    (new Message("FAIL","Unable to Delete"),
    ...
    public ResponseEntity<Message> updateStudent(
        ...
        try {
            ...
            if(exist) {
                ...
                resp=new ResponseEntity<Message>
                        (new Message("OK",student.getStdId()+"-Updated"),HttpStatus.OK);
            }else {
                resp=new ResponseEntity<Message>
                        (new Message("OK",student.getStdId()+"-Not Exist"),
                                HttpStatus.BAD_REQUEST);
            }
        } catch (Exception e) {
            resp=new ResponseEntity<Message>
                    (new Message("OK","Unable to Update"),
                    HttpStatus.INTERNAL_SERVER_ERROR);
            e.printStackTrace();
        }
        return resp;
